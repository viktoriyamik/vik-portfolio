...
const cases: Case[] = [
  ...
  {id:'aura',name:'Aura',category:'Luxury retail · AI concierge concept',headline:'An AI concierge that turns uncertainty into confidence.',summary:'AURA Atelier explores an emotionally intelligent shopping journey for luxury clients. A conversational concierge learns the occasion and style preferences, curates looks and connects the client to a boutique advisor when human expertise matters.',role:'Product design · UX research · UX/UI design',platform:'Mobile prototype · Figma',status:'Design concept',challenge:'Luxury decisions involve social context, identity and confidence. AURA needed to offer relevant suggestions without feeling like a pushy sales chatbot or replacing the relationship with a human advisor.',process:['Framed the strategic challenge and developed a priority persona and emotional journey map.','Mapped the conversational flow from an occasion and preferences to curated looks and a boutique appointment.','Prototyped mobile conversations, recommendations and an advisor handoff, then documented a usability test plan and success criteria.'],decisions:[{title:'Start with the occasion',text:'The conversation asks how the client wants to feel and what social setting they are preparing for.'},{title:'Curate, then explain',text:'A small set of looks supports comparison and gives the client control over the final decision.'},{title:'Hand off with context',text:'When judgment is uncertain, a human advisor receives the conversation context so the client does not need to repeat it.'}],outcome:'AURA Atelier is a prototype and proposed test plan. The Figma deck specifies intended evaluation criteria; it does not report completed user testing or product outcomes.',links:[{text:'Explore Aura in Figma',url:`${figma}?node-id=0-1&t=9kXJZFCfY4z6CPPO-1`}]},
...
const images:Record<ID,{src:string;alt:string;caption:string}[]>={
 ...
 aura:[
  {src:'/cases/aura-curated.png',alt:'AURA Atelier prototype presentation showing curated outfit suggestions and mobile conversation screens',caption:'Curated suggestions in the mobile prototype'},
  {src:'/cases/aura-handoff.png',alt:'AURA Atelier prototype presentation showing a handoff to a human boutique advisor',caption:'Handoff from AI guidance to a human advisor'},
 ],
 cleanpeer:[
...
function App(){
 ...
 return <div className="site">
  ...
  {current?<main className="case" id="top">
   ...
   <section className="case-section"><span>04 / SELECTED SCREENS</span><h2>Explore the experience</h2><div className={`case-gallery gallery-${current.id}`}>{images[current.id].map(image=><figure key={image.src}><img src={image.src} alt={image.alt} loading="lazy"/><figcaption>{image.caption}</figcaption></figure>)}</div></section>
   ...
   <section className="portfolio-columns" id="work"><div className="featured"><p className="overline">FEATURED PROJECTS / 01—03</p><h2>Selected work</h2><div className="featured-list">{cases.map((c,i)=><a href={`#case-${c.id}`} className="featured-card" key={c.id}>{c.id==='aura'?<img className="featured-image" src="/cases/aura-curated.png" alt="AURA Atelier mobile prototype and curated outfit suggestions"/>:<Visual id={c.id}/>}<div><small>0{i+1} / {c.category}</small><h3>{c.name}</h3><p>{c.headline}</p><span>View case study <ArrowUpRight size={15}/></span></div></a>)}</div></div><Assistant/></section>
